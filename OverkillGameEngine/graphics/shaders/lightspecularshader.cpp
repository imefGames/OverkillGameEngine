...
namespace OK
{
    LightSpecularShader::LightSpecularShader(const OK::char8* shaderName)
        : Shader{ shaderName }
    ...
    void LightSpecularShader::InitializeShader(const RenderingContext& renderingContext, const OK::char8* vertexShaderFile, const OK::char8* pixelShaderFile)
    {
        HRESULT result;
        ...
        result = renderingContext.m_Device->CreateBuffer(&cameraBufferDesc, NULL, &m_CameraBuffer);
        okAssert(SUCCEEDED(result), "Failed to create camera buffer.");
    }

    void LightSpecularShader::ShutdownShader()
    {
    ...
    void LightSpecularShader::SetShaderParameters(const RenderingContext& renderingContext, const ShaderRenderData& shaderRenderData)
    {
        HRESULT result;
        ...
        dataPtr2 = reinterpret_cast<LightBufferData*>(mappedResource.pData);
        ...
        dataPtr2->m_SpecularColor = renderingContext.m_SpecularLightColor;
        dataPtr2->m_SpecularPower = renderingContext.m_SpecularLightPower;
        ...
        result = renderingContext.m_DeviceContext->Map(m_CameraBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
        okAssert(SUCCEEDED(result), "Failed to lock the camera buffer.");

        dataPtr3 = reinterpret_cast<CameraBufferData*>(mappedResource.pData);
        dataPtr3->m_CameraPosition = renderingContext.m_CameraPosition;

        renderingContext.m_DeviceContext->Unmap(m_CameraBuffer, 0);
        bufferNumber = 1;

        renderingContext.m_DeviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_CameraBuffer);
    }

    void LightSpecularShader::RenderShaderInternal(const RenderingContext& renderingContext, const ShaderRenderData& shaderRenderData)
    {
